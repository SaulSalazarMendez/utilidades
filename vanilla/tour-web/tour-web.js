...
function render() {
    ...
    if (this.ipaso == -1 || this.ipaso >= this.pasos.length)
        return;
    ...
    let target = document.querySelector(paso.target);
    let cr = null;
    if (!target) {
        cr = [{
                top: 0,
 ...
 * @param {*} indice
 */
function addListerBoton(btn, indice) {
