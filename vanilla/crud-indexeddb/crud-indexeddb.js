export class CrudIndexeddb {
     ...
     */
    init(nameDB, versionDB, tablas, onError) {
    ...
    delete(_table, _key) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                let tx;
                if (!db) {
                    obj.onError({ type: 'DELETE', error: 'no esta abierta la base de datos' });
                    resolve(false);
                    return;
                }
                tx = db.transaction(_table, 'readwrite');
                let store = tx.objectStore(_table);
    ...
    list(_table) {
        ...
        return new Promise((resolve, reject) => {
            try {
                let db = obj.db;
                if (!db) {
                    obj.onError({ type: 'LIST', error: 'no esta abierta la base de datos' });
                    resolve(false);
                    return;
                }
                let tx = db.transaction(_table, 'readonly');
